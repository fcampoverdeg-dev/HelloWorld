...
void setup() {
  ...
  TFTscreen.begin();
  TFTscreen.fillScreen(
      TFT_WHITE); // Alternative: TFTscreen.background(255, 255, 255);
...
void loop() {
  ...
  // clear screen
  TFTscreen.fillScreen(
      random(0xFFFF)); // Alternative: TFTscreen.background(255, 255, 255);
}
...
void renderJPEG(int xpos, int ypos) {
  ...
  while (JpegDec.read()) {
    ...
    if ((mcu_x + win_w) <= TFTscreen.width() &&
        (mcu_y + win_h) <= TFTscreen.height()) {
      // open a window onto the screen to paint the pixels into
      // TFTscreen.setAddrWindow(mcu_x, mcu_y, mcu_x + win_w - 1, mcu_y + win_h
      // - 1);
      TFTscreen.setAddrWindow(mcu_x, mcu_y, mcu_x + win_w - 1,
                              mcu_y + win_h - 1);
